...
if %errorlevel% neq 0 (
    echo.
    echo  ╔═══════════════════════════════════════╗
    echo  ║  Hermes Portable 需要 WSL 才能运行    ║
    echo  ╚═══════════════════════════════════════╝
    echo.
    echo  请先安装 WSL (Windows Subsystem for Linux):
    echo.
    echo    1. 以管理员身份打开 PowerShell
    echo    2. 运行: wsl --install
    echo    3. 重启电脑
    echo    4. 详情: https://learn.microsoft.com/wsl/install
    echo.
    pause
    exit /b 1
)
...
if %errorlevel% neq 0 (
    echo   Opening config panel in browser...
