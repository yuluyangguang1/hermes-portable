...
> Zero-install, zero-trace, cross-platform [Hermes Agent](https://github.com/NousResearch/hermes-agent) — download, extract, double-click.
> 无需安装的便携 AI Agent · 下载解压即用 · 零痕迹 · 全平台
...
**Extract once, run anywhere** — everything bundled (Python, Node.js, venv, agent source, Web UI). Drop the folder on your Desktop, an external drive, or a USB stick. Double-click, go. The host machine's `~/.hermes/`, `$PATH`, registry — nothing gets touched.
...
- **随处运行** — 放桌面、硬盘、U 盘、移动硬盘、甚至网盘目录都行，放哪就在哪跑
- **零痕迹** — 所有读写都被劫持到程序目录内 `data/` 下，宿主机的真 `~/.hermes/` 完全零接触
- **三平台原生** — macOS（arm64/x64）、Linux（x64/arm64）、Windows（x64）
- **Universal 包** — 单个 zip 带齐三平台 venv，启动器自动识别
- **自带 Web UI** — 浏览器管理会话、技能、定时任务、平台通道（无需额外安装）
- **自更新** — 配置面板内一键升级 hermes-agent
...
| 包 | 适用 | 大小 |
|----|------|------|
| `HermesPortable-Windows.zip` | 仅 Windows | ~240 MB |
| `HermesPortable-macOS.zip` | 仅 macOS | ~300 MB |
| `HermesPortable-Linux.zip` | 仅 Linux | ~420 MB |
| `HermesPortable-Universal.zip` | 一包三平台，U 盘 / 移动硬盘多机共用 | ~860 MB |

下载 → 解压到任意位置（本地硬盘、U 盘、移动硬盘都行） → 双击启动器。首次运行会在浏览器打开配置面板（`http://127.0.0.1:17520`），填 API Key 即可。
...
python3 build.py /Volumes/MyUSB     # → 直接输出到 U 盘（或任意目录）
...
适合一份 zip 在多台不同系统的机器上共用（比如放 U 盘或移动硬盘里携带，macOS 和 Windows 交替插用）。
...
1. 解压到**任意位置**——桌面、硬盘、U 盘、移动硬盘都行（Windows 注意避开深路径和中文路径，且不要放在 OneDrive / iCloud Drive 等同步目录里）
2. 双击启动器：
...
于是所有走 `~/.hermes` 的读写（hermes-web-ui 缓存、pip config、Node 的 npm/npx 等）**全部落到程序目录内的 `data/` 里**，宿主机真 `~/.hermes/`、`%USERPROFILE%\.hermes` 从头到尾**零接触**。

删除 HermesPortable 文件夹（或拔掉 U 盘）→ 什么都带走，什么都没留下。
