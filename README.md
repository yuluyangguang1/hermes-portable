...
- **零痕迹** — 所有读写都被劫持到 U 盘内 `data/` 目录，宿主机的真 `~/.hermes/` 完全零接触
...
- **可视化配置中心** — 选模型/填 Key/测试连接/换模型/查看日志，全部在浏览器里
- **运行状态指示** — 配置中心显示 Hermes 进程状态、PID、Web UI 入口
- **配置导入导出** — 一键备份/迁移配置到其他设备
- **自动更新** — 支持 git 用户和 release zip 用户两种更新模式
- **断连检测** — 后端进程退出时配置中心自动提示
